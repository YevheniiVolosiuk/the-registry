---
name: registry
description: Private skill distribution system. Use when the user wants to install, use, add, push, remove, sync, list, or search for skills, agents, or prompts from their private registry catalog. Triggers on /registry commands or mentions of registry, skill distribution, or agentic management.
...
The Registry is a catalog of references to your agentics. The `registry.yaml` file points to where skills, agents, and prompts live (local filesystem or GitHub repos). Nothing is fetched until you ask for it.
